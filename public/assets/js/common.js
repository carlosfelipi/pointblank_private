...
$(function(){
	...
	function eventreset(device){
		...
		$('.m_close, .gnb > h1, .login_box > h1, .gnb .quick_sns, .gnb .nation_select').remove();
		$('.m_menu, .mp .gnb > ul > li.in_ul > a, .m_user, .tool00').unbind('click');
	...
	function mbmenu(device){
		...
		$('.m_menu').click(function(){
			...
			$('.m_close, .gnb > h1, .gnb .quick_sns, .gnb .nation_select').remove();
		...
		$('.m_user').click(function(){
			...
			$('.mp .login_box').prepend('<a class="m_close" href="#none"><img src="/images/btn_close_m.png" alt="mene_close"></a>');
	...
	function vodpop(){
		$('.new_close').click(function(){
			...
			$('.vod_link').click(function(){
				...
				var txt = "<iframe width='100%' height='100%' src='"+_vurl+"' frameborder='0' allowfullscreen></iframe>";
				$(".new_video").html(txt);
